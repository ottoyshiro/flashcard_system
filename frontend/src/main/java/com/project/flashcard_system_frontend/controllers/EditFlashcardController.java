...
public class EditFlashcardController {
    ...
    public void setFlashcard(FlashcardDTO dto) {
        ...
        contentField.setText(dto.getContent());
        subjectField.setText(dto.getSubject());
        ...
        for (int i = 0; i < dto.getAnswers().size(); i++) {
            ...
            TextField field = new TextField(answer);
            ...
            int index = i;
            if (index == dto.getCorrectAnswer()) {
                rb.setSelected(true);
            }

            removeBtn.setOnAction(e -> answersBox.getChildren().remove(box));

            box.getChildren().addAll(rb, field, removeBtn);
    ...
    private void handleSave() {
        ...
        dto.setId(currentFlashcard.getId());
        dto.setContent(content);
        dto.setSubject(subject);
        dto.setAnswers(answers);
        dto.setCorrectAnswer(correctAnswer);

        boolean success = ApiService.editFlashcard(dto);

        if (success) {
            AnimationFeedback.showTimedFeedback(feedback, "Flashcard edited successfully!", "green-feedback", 0.5);
            goToFlashcardList();
        } else {
            System.out.println("Falha ao editar flashcard.");
        }
    }
